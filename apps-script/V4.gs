/* Plansul — extensão backend V4 para Aplicações Financeiras.
 * No projeto Google Apps Script, mantenha este arquivo junto do Code.gs. */
function applicationIdentityServerV4(contaCod, banco, fundo){
  const cc = String(contaCod||'').trim();
  if(cc) return 'cc:' + normalizeAccountServer(cc);
  return 'bf:' + normalizeAccountServer(banco) + '|' + normalizeAccountServer(fundo);
}
...
function readApplications(){
  ...
  for(let i=1;i<data.length;i++){
    ...
    funds.push({
      id:String(row[0]), fundKey:applicationIdentityServerV4(contaCod,banco,fundo), banco, fundo, contaCod, competencia,
  ...
  const latestByFund = {};
  funds.forEach(f=>{ const k=f.fundKey; if(!latestByFund[k] || f.competencia>latestByFund[k].competencia) latestByFund[k]=f; });
...
function doSaveApplications(body){
  ...
  try{
    ...
    const periodicMap = {};
    previousValues.forEach(row=>{const key=applicationIdentityServerV4(row[3],row[1],row[2]);const accepted=String(row[19]||'').toLowerCase()==='true'||row[19]===true;if(accepted)periodicMap[key]=true;});
    const now = new Date().toISOString();
    const values = funds.map(f=>{
      const key=applicationIdentityServerV4(f.contaCod,f.banco,f.fundo), periodicAccepted=!!f.periodicAccepted||!!periodicMap[key];
...
function setupSpreadsheetV4(){
  ...
  SpreadsheetApp.getUi().alert('Estrutura V4 de Aplicações Financeiras atualizada com sucesso.');
}
